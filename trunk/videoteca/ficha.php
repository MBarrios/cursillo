...
echo "<h2 class='colorRojo'>FICHA DE PELICULAS</h2>";
...
if(!isset($_GET["id"]))
echo "<p>BUSCADOR DE PELICULAS</p>";
else
echo "<h2 class='colorRojo'>BUSCADOR DE PELICULAS</h2>";
...
?>
  ...
  <select name="titulos" id="titulos" onchange="mostrar_ficha();">
	<option value=0>Elija una pelicula</option>
  <?php
  ...
  while($fila=mysql_fetch_array($resultado))
  	if($_GET["id"]==$fila["id"])
  	//para que cuando se carge la pagina de nuevo siga manteniendo la seleccion
