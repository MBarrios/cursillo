<?php
function crearTabla($_pagi_sql,$arrayCabecera,$numCol){
...
while($fila=mysql_fetch_array($_pagi_result)){
	...
	//si queremmos que la primera columna tenga enlaces
		...
		if($i==$numCol && stripos($pagina, $patron2)==true){
			if($i==4)
				echo "<td><a href='fichaReceta.php?id=".$fila["0"]."'>Ver receta</a></td>";
			else
				echo "<td><a href='fichaReceta.php?id=".$fila["0"]."'>".$fila[$i]."</a></td>";
		}
		else
			echo "<td>".$fila[$i]."</td>";
		$i=$i+1;
